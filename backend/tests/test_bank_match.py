...
def test_rule_beats_keywords():
    rules = [{"keyword": "grab", "bank": "superbank", "category_id": 2}]
    cats = [
        {"id": 1, "type": "expense", "keywords": '["grab"]'},
        {"id": 2, "type": "expense", "keywords": "[]"},
    ]
    assert suggest_category_id(rules, cats, "superbank", "QRIS GRAB", "expense") == 2


def test_keywords_fallback():
    cats = [{"id": 3, "type": "expense", "keywords": '["alfamart"]'}]
    assert suggest_category_id([], cats, "bca", "Debit di Alfamart", "expense") == 3


def test_wrong_type_ignored():
    cats = [{"id": 3, "type": "income", "keywords": '["gaji"]'}]
    assert suggest_category_id([], cats, "jago", "gaji masuk", "expense") is None
